...
export function Badge({ variant, label, text }: BadgeProps) {
  ...
  return (
    <View className={`flex-row items-center ${style.bg} gap-1 self-start rounded-full px-2 py-0.5`}>
      {style.icon && <Ionicons name={style.icon as never} size={11} color={style.iconColor} />}
      {displayText && (
        <Text className={`text-[11px] font-semibold ${style.text}`}>{displayText}</Text>
      )}
    </View>
  );
}
