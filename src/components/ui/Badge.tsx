...
type BadgeVariant = 'verified' | 'online' | 'offline' | 'tier' | 'status' | 'count' | 'primary' | 'warning' | 'error' | 'success';
...
export function Badge({ variant, label, text }: BadgeProps) {
  ...
  return (
    <View className={`flex-row items-center ${style.bg} px-2 py-0.5 rounded-full self-start gap-1`}>
      ...
      {displayText && <Text className={`text-[11px] font-semibold ${style.text}`}>{displayText}</Text>}
    </View>
  );
}
