...
export function ReviewCard({ review, expanded = false }: ReviewCardProps) {
  ...
  return (
    <View className="border-b border-gray-100 py-3.5">
      <View className="mb-2 flex-row items-center">
        <Avatar name={review.reviewerName} photoUrl={review.reviewerPhotoUrl} size={38} />
        <View className="ml-2.5 flex-1">
          <Text className="text-sm font-semibold text-gray-900">{review.reviewerName}</Text>
      ...
      {review.comment ? <Text className="text-sm leading-5 text-gray-700">{text}</Text> : null}
    </View>
  );
}
