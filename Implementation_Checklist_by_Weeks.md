...
- [x] Polish and refine Client app screens with final component integration
  - [x] Replace any inline/ad-hoc styles with `theme.ts` token classes
  - [x] Add loading skeletons to Search results and Bookings list
  - [x] Add visible empty states to Search (no results) and Bookings (no history)
  - [x] Verify `KeyboardAvoidingView` behavior on all form screens (Login, KYC-lite)
- [x] Polish and refine Handyman app screens with final component integration
  - [x] Consistent header layout with Online/Offline `Badge` across all Handyman tabs
  - [x] Animate state transition on active job card CTA change
  - [x] Loading state on wallet balance fetch; error state on network failure
...
- [x] End-to-end interactive navigation test across all role flows
  - [x] Manual test pass: Client full booking flow (search → book → status tracking)
  - [x] Manual test pass: Handyman full flow (receive request → accept → complete job)
  - [x] Manual test pass: Admin KYC approval and dispute navigation
