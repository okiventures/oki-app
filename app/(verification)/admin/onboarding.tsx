                    ...
                    <View className="mb-8 w-full rounded-xl bg-amber-50 p-4">
                        <View className="flex-row items-start gap-3">
                            ...
                            <Text className="flex-1 text-[12px] leading-5 text-amber-800">
                                Typical review takes 1-2 business days. You&apos;ll get access to the admin panel once approved.
                            </Text>
                        </View>
                    </View>
        ...
        <View className="mb-6 flex-row items-center gap-2">
            {STEPS.map((s, i) => (
                <View key={s.key} className="flex-1 flex-row items-center">
                    <View
                        className={`h-7 w-7 items-center justify-center rounded-full ${i <= stepIndex ? 'bg-indigo-600' : 'bg-gray-200'
                            }`}>
                        <Text
                            className={`text-[11px] font-bold ${i <= stepIndex ? 'text-white' : 'text-gray-500'
                                }`}>
                            {s.number}
                        </Text>
                    </View>
                    {i < STEPS.length - 1 && (
                        <View className={`mx-1 h-0.5 flex-1 ${i < stepIndex ? 'bg-indigo-600' : 'bg-gray-200'}`} />
                    )}
                </View>
            ))}
        </View>
        ...
        <>
            ...
            <Form gap={3}>
                <Input
                    ...
                    error={fullName.length > 0 && !personalValid.fullName ? 'Use at least 2 characters.' : undefined}
        ...
        <>
            ...
            <View className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
                <View className="flex-row items-start gap-3">
                    ...
                    <View className="flex-1">
                        <Text className="text-[13px] font-semibold text-amber-800">Company Verification Required</Text>
                        <Text className="mt-1 text-[12px] leading-5 text-amber-700">
                            A valid company code is required. Contact your company admin if you don&apos;t have one.
                        </Text>
                    </View>
                </View>
            </View>
            ...
            <Form gap={3}>
                <Input
                    ...
                    error={companyName.length > 0 && !companyValid.companyName ? 'Use at least 2 characters.' : undefined}
                ...
                <Input
                    ...
                    error={companyCode.length > 0 && !companyValid.companyCode ? 'Code must be 3+ characters.' : undefined}
                ...
                <Input
                    ...
                    error={companyAddress.length > 0 && !companyValid.address ? 'Enter a full address (5+ chars).' : undefined}
                    leftIcon={<Ionicons name="location-outline" size={18} color="#9CA3AF" />}
                />
            </Form>

            <View className="mt-6 flex-row gap-3">
                <TouchableOpacity onPress={goBack} className="flex-1 items-center rounded-xl border border-gray-300 py-3">
                ...
                {ADMIN_ROLES.map((r) => {
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            className={`rounded-xl border-2 p-4 ${active ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white'
                                }`}>
                            <View className="flex-row items-center gap-3">
                                <View
                                    className={`h-5 w-5 items-center justify-center rounded-full border-2 ${active ? 'border-indigo-500' : 'border-gray-300'
                                        }`}>
                                    {active && <View className="h-3 w-3 rounded-full bg-indigo-500" />}
                                </View>
                                <View className="flex-1">
                                    <Text className={`font-semibold text-[14px] ${active ? 'text-indigo-900' : 'text-gray-900'}`}>
                                        {r.label}
                                    </Text>
    ...
    const renderReview = () => (
        <>
            ...
            <Card className="mb-4 p-4">
                <Text className="mb-2 text-[12px] font-semibold uppercase tracking-wider text-gray-500">
                    Personal Info
                </Text>
                ...
                <View className="gap-1">
                    <ReviewRow icon="shield-outline" label="Role" value={ADMIN_ROLES.find((r) => r.id === selectedRole)?.label || selectedRole} />
                    {department ? <ReviewRow icon="folder-outline" label="Department" value={department} /> : null}
                </View>
            </Card>
            ...
            <TouchableOpacity
                ...
                <View
                    className={`mt-0.5 h-5 w-5 items-center justify-center rounded border-2 ${agreed ? 'border-indigo-500 bg-indigo-500' : 'border-gray-300'
                        }`}>
