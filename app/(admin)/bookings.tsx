...
const BOOKING_STATUS_OPTIONS = ['All', 'Pending', 'Accepted', 'InTransit', 'Arrived', 'WorkStarted', 'Completed', 'Paid', 'Cancelled'];
    ...
    return (
        <View className="flex-1 bg-gray-50">
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    <>
                        <Text className="text-[13px] font-bold text-gray-900 mb-3 px-1">Booking queue and history</Text>
                        <SearchBar
                            value={searchValue}
                ...
                ListEmptyComponent={
                    <Text className="text-[12px] text-gray-500 px-1">No bookings match your search or filter.</Text>
                }
                renderItem={({ item }) => {
                        ...
                        <TouchableOpacity
                            ...
                            <Card className="p-4">
                                <View className="flex-row items-center justify-between mb-2">
                                    <Text className="text-[15px] font-bold text-gray-900">{item.serviceCategory}</Text>
                                ...
                                <View className="flex-row items-center justify-between mb-2">
                                    ...
                                    <Text className="text-[12px] text-gray-500">{formatDate(item.scheduledAt || item.createdAt)}</Text>
                                </View>
                                <View className="flex-row items-center justify-between">
                                    <Text className="text-[13px] font-semibold text-gray-900">{formatCurrency(item.amount)}</Text>
                                    <Text className="text-[12px] text-gray-500">Net {formatCurrency(item.netAmount)}</Text>
                                </View>
                                {isExpanded && (
                                    <View className="mt-3 space-y-2 border-t border-gray-100 pt-3">
                                        ...
                                        <Text className="text-[12px] text-gray-600">Status updated: {formatDate(item.updatedAt)}</Text>
                                    </View>
                                )}
                            </Card>
                        </TouchableOpacity>
