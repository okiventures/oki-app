  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div>
          <p className="mb-3 text-[13px] font-bold text-gray-900">
            Pending KYC Verifications
          </p>
          {MOCK_KYC_REQUESTS.map((item) => (
            <Card
              key={item.id}
              className="mb-3 flex flex-row items-center p-3"
            >
              <div className="h-8 w-8 items-center justify-center rounded-full bg-gray-100 flex">
              ...
              <div className="flex items-end gap-2">
                ...
                <button className="text-primary-600 text-[11px] font-bold">
                  Review
                </button>
              </div>
            </Card>
              ...
              return (
                ...
                >
                  <div className="flex flex-row items-center justify-between">
                    ...
                    <Badge
                      text={user.status}
                      variant={statusBadge(user.status)}
                    />
                  </div>
                  {isSelected && (
                    <div className="mt-3 space-y-2 border-t border-gray-100 pt-3">
                      ...
                      <p className="text-[12px] text-gray-600">
                        Last active{" "}
                        {formatDate(user.lastActive || user.createdAt)}
                      </p>
                      <div className="flex flex-row gap-2">
                        <button
                          className="flex-1 items-center rounded-xl bg-gray-100 py-2 text-[12px] font-semibold text-gray-700"
                        >
                          Suspend
                        </button>
                        <button
                          className="bg-primary-600 flex-1 items-center rounded-xl py-2 text-[12px] font-semibold text-white"
                        >
                          Message
                        </button>
                      </div>
